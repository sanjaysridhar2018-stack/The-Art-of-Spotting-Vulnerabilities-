...
### XSS
Unlike SQLi where I already had grasped the basics of SQL before, here I faced a real challenge since I had no idea about Javascript, making my fundamentals weak. Now CTFs are where the real application part begins, one major learning is to first deploy a random search query and analysing the page source. This gives you an idea whether the input is converted into HTML attribute, if it is embedded in an image or if words like script are banned. This gives you the pathway to think and proceed with the next step rather than directly trying to inject. If I were stuck in a HTML attribute my first question would be how to escape it rather than how i would inject it. 
### CSRF
### SSRF
